"""Local Gemma capability probe via Ollama.

Same probes as gemma_benchmark.py, but talks to the local Ollama HTTP endpoint
instead of Google's hosted API. Run for each model size you have pulled to
compare quality/latency progression vs hosted 31B.

Run: `uv run python docs/future/tools/gemma_local_benchmark.py <model_tag>`
e.g.  `uv run python docs/future/tools/gemma_local_benchmark.py gemma3:4b`
"""
...
def call(model: str, prompt: str, max_tokens: int = 400) -> tuple[str, float]:
    payload = {
        "model": model,
        "prompt": prompt,
        "stream": False,
    ...
    req = request.Request(OLLAMA_URL, data=body, headers={"Content-Type": "application/json"})
    t0 = time.perf_counter()
    try:
        with request.urlopen(req, timeout=600) as resp:
            data = json.loads(resp.read().decode("utf-8"))
    except URLError as e:
        return f"<<URLError: {e}>>", time.perf_counter() - t0
...
def probe_personas(model: str) -> dict[str, tuple[str, float]]:
    print("\n=== PROBE 1-3: PERSONA DISTINCTNESS ===\n")
    out: dict[str, tuple[str, float]] = {}
    for name, persona in PERSONAS.items():
        prompt = f"{persona}\n\n{SCENARIO}\n\nWrite your position now."
        text, dt = call(model, prompt, max_tokens=320)
        out[name] = (text, dt)
        print(f"--- {name} ({dt:.2f}s) ---\n{text}\n")
    return out


def probe_synthesizer(model: str, persona_outputs: dict[str, tuple[str, float]]) -> None:
    print("\n=== PROBE 4: SYNTHESIZER ===\n")
    bundle = "\n\n".join(
    ...
    text, dt = call(model, prompt, max_tokens=400)
...
def probe_narration(model: str) -> None:
    ...
    text, dt = call(model, prompt, max_tokens=180)
    print(f"--- narration ({dt:.2f}s) ---\n{text}\n")
    fabricated_terms = ["earnings call", "guidance", "Azure", "Activision", "AI revenue", "buyback"]
    found = [t for t in fabricated_terms if t.lower() in text.lower()]
    print(f"Fabricated-fact check: {found or 'clean'}")
    print(f"Mentions composite={('58' in text or '58.4' in text)}, threshold={('65' in text)}")


def probe_hallucination_resistance(model: str) -> None:
...
if __name__ == "__main__":
    ...
    persona_out = probe_personas(model)
    probe_synthesizer(model, persona_out)
    probe_narration(model)
    probe_hallucination_resistance(model)
    print("\n=== DONE ===")
