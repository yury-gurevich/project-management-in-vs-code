...
**Version:** 1.1 (skill package v0.7)
**Date:** 2026-04-26
...
│   ├── memory/                   in-skill cross-session memory (4-type taxonomy)
│   │   ├── README.md             explainer + when-to-write rules
│   │   ├── _TEMPLATE.md          new-entry template
│   │   ├── MEMORY.md             one-line index of entries
│   │   └── <type>_<topic>.md     individual entries (gitignored — per-install only)
...
Resolved in skill package v0.4 (Phase C):
...
Resolved in skill package v0.7 (Phase C):

- **Q (was Q1):** Existing projects without a PRD — how does the skill handle them? **Resolved (C5, 2026-04-26):** offer both modes during bootstrap. `/bootstrap` pre-flight now branches three ways: PRD-exists (today's behavior), no-PRD-trivial-repo (< 5 commits AND ≤ 10 files AND no/short README → fresh-start interview), no-PRD-non-trivial-repo (offer reverse-engineer / fresh-start / cancel). Reverse-engineer mode surveys git log + file tree + README + manifest files, drafts a best-effort PRD with explicit `[?]` and `[inferred — confirm]` markers + ≥ 3 mandatory Open Questions, presents it with explicit "nothing is saved yet" framing, then runs a short interview to validate before writing. Hard approval gate before any file is created. Project-log starts at bootstrap — pre-existing work stays in git history (no fabricated retroactive `started`/`shipped` entries; new principle in `bootstrap/SKILL.md`: "Don't invent the past"). See `bootstrap/SKILL.md` Pre-flight + Reverse-engineer mode sections.

Still open for v1.x:

1. **Ideas parking lot integration with completion plan** — when a parked idea graduates to a committed stage, does the skill do that automatically or only via `/replan`? *Lean: only via `/replan`; automation here would undermine the discipline.*
2. **Auto-trigger collisions** — when two skills could plausibly fire on the same phrasing (e.g., "let's plan the next thing" — `/replan` or just generic next-stage selection?), how does the agent disambiguate? *Lean: defer to the most-specific description match; if tied, ask one clarifying question before acting.*
...
- **2026-04-26 — C5 (skill package v0.7).** Closed open question Q1 (existing projects without a PRD). `/bootstrap` pre-flight now branches three ways (PRD-exists / no-PRD-trivial-repo / no-PRD-non-trivial-repo) with a configurable non-trivial-repo heuristic (≥ 5 commits OR ≥ 10 source files OR README ≥ 500 chars). Non-trivial repos get a **Reverse-engineer mode** that surveys `git log -50` + `git log --stat --since='6 months ago'` + `git ls-tree -r HEAD` + README + language manifest, then drafts a best-effort PRD with `[?]` and `[inferred — confirm]` markers, ≥ 3 mandatory Open Questions, and an explicit approval gate before writing. Two new principles added to `bootstrap/SKILL.md`: "Reverse-engineer is best-effort, never silent" and "Don't invent the past" (project-log starts at bootstrap; no fabricated retroactive entries). bootstrap SKILL.md description updated with reverse-engineer trigger phrases for auto-trigger reliability (per principle 8 / PL-G7). §9 reorganized: Q1 moved to "Resolved in skill package v0.7"; remaining open questions renumbered. Reason: G1 (vibe-coder bootstrap ≤ 15 min) and G4 (portable across stacks) both fail when the user is sitting on real code and the only path is "throw it all away and start over."
...
*End of PRD v1.1 / project-log v0.7.*
